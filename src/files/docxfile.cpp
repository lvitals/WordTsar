...
bool cDOCXFile::CheckType(QString filename)
{
    QString ext;

    // Encontrar a posição do último '.' no nome do arquivo
    int found = filename.lastIndexOf(".");
    ext = filename.mid(found + 1);

    // Converter a extensão para minúsculas
    ext = ext.toLower();

    // Verificar se a extensão é 'docx'
    return (ext == "docx");
}
...
bool cDOCXFile::LoadFile(QString filename)
{
    bool retval = false;
    ...
    zip_t *zip = zip_open(filename.toStdString().c_str(), ZIP_DEFAULT_COMPRESSION_LEVEL, 'r');
    if (zip != nullptr)
    {
        // open Style Sheets
...
bool cDOCXFile::SaveFile(QString filename, POSITION_T length)
{
    bool retval = false;
    return retval;
}
...
QString cDOCXFile::GetExtensions(void)
{
