        ...
        if (element[loop]->mType == eRTFTypeGroup)
        {
            ...
            for (size_t attribloop = 0; attribloop < group->mChildren.size(); attribloop++)
            {
                ...
                if (felement->mType == eRTFTypeControlWord)
                {
                    ...
                    // check font family
                    if (control->mWord == "fnil" || control->mWord == "froman" || control->mWord == "fswiss" || control->mWord == "fmodern" || control->mWord == "fscript" || control->mWord == "fdecor" || control->mWord == "ftech" || control->mWord == "fbidi")
                    {
