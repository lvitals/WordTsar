    ...
    : QMainWindow(parent)
{
    ...
    // mLayout->setAlignment(mHelpCtrl, Qt::AlignHCenter);
    

    mHelpJCtrl = new cEditorCtrl(mBaseWidget);
    mHelpJCtrl->setObjectName(QStringLiteral("mHelpJCtrl"));
    mHelpJCtrl->setMaximumSize(QSize(16777215, 100));
    ...
    string helpjj("                  ----- JIFFY MENU -----\n"
                  " "
                  "    JIFFY FUNCTIONS                   INSERT\n"
                  " P play          E rename        @ today's date         * current filename\n"
                  " R record        O copy          ! current time         : current drive\n"
                  " D edit/create   Y delete        = last math result     . current directory\n"
                  " S single step                   # last math expression \\ current path\n"
                  "                                 $ last math as dollar");
    mHelpJCtrl->SetAsHelp();
    mHelpJCtrl->InsertWordStarString(helpjj);
...
void cWordTsar::UpdateStatus(cEditorCtrl *editor)
{
    ...
    if (cstatus.help == HELP_CTRLJ && lasthelp != HELP_CTRLJ)
    {
        counter = 0;
        lasthelp = HELP_CTRLJ;
    }

    if (cstatus.help == HELP_CTRLJ && counter >= HELPDELAY)
    {
        mHelpCtrl->hide();
        mHelpJCtrl->show();
        mHelpKCtrl->hide();
...
void cWordTsar::CreateMenus(void)
{
    ...
    */
    ...
    QAction *GotoCharAction = new QAction("&Go to Character...\t^QG", this);
