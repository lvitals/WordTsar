...
void cEditorCtrl::keyPressEvent(QKeyEvent *event)
{
    ...
    // Verifica se os objetos essenciais não são null
    if (!mLayout || !mInput) {
        return;
    }
...
//    }
    ...
    case Qt::Key_Delete:
        if (mInput->CheckControlMode() == false)
        {
            DeleteChar();
            done = true;
        }
        // a hack so we can handle things like ^QDel
        else
        {
            del = true;
        }
    ...
    if (!done)
    {
        ...
        // if we are in a control mode, the delete key gets converted into a space
        if (del == true)
        {
            text = " ";
        }
        ...
        if (!text.isEmpty()) {
            // Verifique se o texto não está vazio antes de acessar o primeiro caractere
            CHAR_T ch = text.at(0).unicode();  // Pode ser necessário lidar com caracteres maiores que char

            // O HandleKey espera um char, então você deve garantir que o valor Unicode seja convertido corretamente
            bool handled = mInput->HandleKey(static_cast<char>(ch), shift);  // Verifique se a função HandleKey lida bem com esse cast
            ...
            if (!handled) {
                ...
                if (mInsertMode == OVERWRITE_MODE) {
                    DeleteChar();  // Deleta o caractere sob o cursor antes de sobrescrever
                }

                // Insere o caractere no documento
                mDocument.Insert(ch);
            }
        }
...
void cEditorCtrl::SetFrame(cWordTsar *frame)
{
    ...
    string title;
    title = string_sprintf("%s - WordTsar %ld.%ld build %ld %s", mFileName.c_str(), MAJOR, MINOR, BUILD, STATUS);
...
bool cEditorCtrl::LoadFile(QString filename)
{
    ...
    QString oldfilename = mFileName.c_str();
    ...
    for (auto &mFileType : mFileTypes)
    {
        ...
        {
            // Use QString em vez de std::string
            if (mFileType->CheckType(filename))
            {
                SetStatus("Loading...");
                ...
                // Passe filename como QString
                retval = mFileType->LoadFile(filename);
...
bool cEditorCtrl::SaveFile(QString filename)
{
    ...
    for (size_t loop = 0; loop < mFileTypes.size(); loop++)
    {
        ...
        {
            // Use filename diretamente como QString
            if (mFileTypes[loop]->CheckType(filename) == true)
            {
                // Converta filename para std::string apenas quando necessário
                retval = mFileTypes[loop]->SaveFile(filename, size);
                break;
            }
        }
    }
...
void cEditorCtrl::EmergencySaveFile(void)
{
    ...
    QString savefile;
    QFileInfo filename(mFileName.c_str());

    string sname = mFileDir + filename.completeBaseName().toStdString() + "-error." + filename.completeSuffix().toStdString();
    savefile = sname.c_str();
    SaveFile(savefile);
...
void cEditorCtrl::OnStatusTimer(void)
{
    ...
    if (savecounter == 300) // we call this 5 times a second, so 1 minute is 300 calls
    {
        ...
        if (mFileName.length() != 0)
        {
            ...
            string sname = mFileDir + "/" + filename.completeBaseName().toStdString() + "-bak." + filename.completeSuffix().toStdString();
            savefile = sname.c_str();
            SaveFile(savefile);
        }
        // #endif
...
void cEditorCtrl::MoveDown(void)
{
    ...
    // If we are not showing dot commands, then skip the appropriate lines
    if (GetShowDot() == false)
    {
    ...
    // Check if the next line is within the document limits
    if (currentline + nextlineoffset < maxlines)
    {
        ...
        // If the new line is shorter than the current caret position, adjust
        if (endlinepos - newlinepos < pos - curlinepos)
        {
            pos = endlinepos;  // Adjust the cursor to the end of the line
        }
        else
        {
            pos = newlinepos + (pos - curlinepos);  // Maintain the same relative position on the new line
        }

        // Ensure the cursor does not exceed the document size
        if (pos >= mDocument.GetTextSize())
        {
            pos = mDocument.GetTextSize() - 1;  // Adjust to the end of the document
        }

        // Get the character at the new position to check for hidden controls
        mDocument.GetChar(pos);

        // Update the cursor position in the document
        mDocument.SetPosition(pos);
...
void cEditorCtrl::DeleteLineLeft(void)
{
    POSITION_T pos = mDocument.GetPosition();
    LINE_T line = mLayout->GetLineFromPosition(pos);
    POSITION_T start = mLayout->GetLineStartPosition(line);
...
void cEditorCtrl::DeleteLineRight(void)
{
    POSITION_T pos = mDocument.GetPosition();
    LINE_T line = mLayout->GetLineFromPosition(pos);
    POSITION_T end = mLayout->GetLineEndPosition(line);
...
void cEditorCtrl::DeleteToChar(void)
{
    ...
    if (ecode != 0)
    {
        ...
        QString ch;
        ch.append(gotochar.character->text()[0]);
        if (ch.length() != 0)
        {
            mWholeFile = false;
            mCaseCmp = false;
...
void cEditorCtrl::InvalidCommand(string t)
{
    QString t1 = strformat("Invalid Command %s ", t.c_str()).c_str();
    QMessageBox msgBox(QMessageBox::Information, "Information", t1, QMessageBox::Ok, this);
    msgBox.exec();
}
...
void cEditorCtrl::Replace(void)
{
    ...
    if (ecode != 0)
    {
        mLastFindandReplace = mDocument.GetPosition(); // remember where we were before we started our search
...
void cEditorCtrl::GotoCharacter(void)
{
    ...
    dialog.setWindowTitle("Goto to Charcater");
    int ecode = dialog.exec();
...
void cEditorCtrl::GotoPage(void)
{
    ...
    if (ecode != 0)
    {
        ...
        if (page != 0)
        {
            if (page < mLayout->GetNumberofPages())
            {
                size_t numparas = mLayout->mParagraphLayout.size();

                POSITION_T cpos = mDocument.GetPosition();
                POSITION_T newpos = 0;

                /// @todo we probbaly want to optimize this

                // go through each paragraph
                LINE_T currentline = 0;
                for (size_t ploop = 0; ploop < numparas; ploop++)
                {
                    ...
                    for (sLineLayout line : paragraph->lines)
                    {
                        ...
                        {
                            newpos = mLayout->GetLineStartPosition(currentline);
                            mDocument.SetPosition(newpos);
                            ploop = numparas; // break from outer loop
                            break;
                        }
                ...
                //                mDocument.SetPosition(mDocument.GetPagePosition()) ;
...
void cEditorCtrl::GotoFontTag(void)
{
    POSITION_T pos = mDocument.GetNextFontTagPosition();
    POSITION_T curr = mDocument.GetPosition();
...
void cEditorCtrl::GotoPreviousPosition(void)
{
...
void cEditorCtrl::GotoLastFindandReplace(void)
{
    mDocument.SetPosition(mLastFindandReplace);
...
{
    status.filename = mFileName.c_str();
...
POSITION_T &cEditorCtrl::DisplayListContolChars(sSegmentLayout &segment, size_t loop1, COORD_T x, POSITION_T &paraposition, POSITION_T caratposition, QPainter *paint, sLineLayout &line, sDisplayList &newdisplay, vector<QRectF> &rects, bool dot)
{
    ...
    if (segment.glyph[loop1][0] < STYLE_END_OF_STYLES && segment.glyph[loop1].length() == 1)
    {
        if (segment.glyph[loop1][0] != HARD_RETURN)
        {
            ...
            {
                sWSTab tab = mDocument.GetTab(paraposition);
                // only show if it's a true tab
                if (tab.type == TAB_TAB)
                {
                    if (mShowControl == SHOW_ALL)
                    {
