    ...
    if (ch[0] == '.' && ch.length() == 1)
    {
        ...
        if (mParagraphEnd - mParagraphStart > 1)
        {
            ...
            else // support previous .ig command as well as ..
            {
                ...
                if ((checkchar == "i" || checkchar == "I") && (checkg == "g" || checkg == "G"))
                {
                    mIsCommand = false;
                    mIsComment = true;
                    mIsKnownDotCommand = true;
...
void cLayout::WordWrapParagraph(COORD_T &pageextend, PARAGRAPH_T paragraph)
{
    ...
    for (size_t sloop = 0; sloop < numsegments; sloop++)
    {
        ...
        for (size_t gloop = 0; gloop < numglyphs; gloop++)
        {
            ...
            {
                POSITION_T posstart, posend;
                mDocument->GetParagraphStartandEnd(paragraph, posstart, posend);
                sWSTab tab = mDocument->GetTab(gloop + posstart);
                // only calculate if it's a true tab
                if (tab.type == TAB_TAB)
                {
...
bool cLayout::SaveLine(bool paCommand, PAGE_T &pagenumber, COORD_T &ypos, COORD_T &tempy, COORD_T &pagetest, sLineLayout &finalline)
{
    ...
    if (mTabCenterLine)
    {
        finalline.center = true;
    }
    else if (mTabRightLine)
    {
        finalline.right = true;
    }
    ...
    if (!mIsComment && !mIsCommand)
    {
        ...
        if (mCenterLine || mRightLine || mJustifyLine || mTabCenterLine || mTabRightLine)
        {
...
void cLayout::ParseTabs(string nums)
{
    ...
    string piece;
    for (size_t loop = 0; loop < nums.length(); loop++)
    {
        if (isdigit(nums[loop]) || nums[loop] == '.')
        {
            piece.push_back(nums[loop]);
        }
        else
        {
            if (piece.length() != 0)
            {
                char valid[] = "CMIP\""; // cm, mm, p, inch(i), "
                auto start = begin(valid);
                auto stop = end(valid);

                for (size_t innerloop = loop; innerloop < nums.length(); innerloop++)
                {
                    bool isvalid = std::find(start, stop, nums[innerloop]) != std::end(valid);
                    if (isvalid)
                    {
                        piece.push_back(nums[innerloop]);
                    }

                    // as soon as we hit eol
                    else if (!isvalid || innerloop == nums.length() - 1)
                    {
                        loop = innerloop - 1;

                        double value = mDocument->GetValue(piece, incdec);
                        char type = mDocument->GetType(piece);
                        double newtab = mDocument->ConvertToTwips(value, type);
